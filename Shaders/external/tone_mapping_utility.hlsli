...
// Ironclad
// The inverse of Reinhard
float3 ITM_Reinhard_Fixed(float3 sdr, float k = 1.0)
{
    // the inverse of (hdr / (hdr + k)) is (k * sdr / (1 - sdr)) which is not what ITM_Reinhard() has
    return k * sdr / (1 - sdr);
}
...
// Ironclad
float3 ITM_Stanard_Fixed(float3 sdr)
{    
    // used when we need to cancel out tonemapping (e.g. we used to have some skybox layers that we didn't want mutated)
    // need to expand 0..1 into hdr space such that then its tone mapped it returns to the painted skybox colors.    
    // ITM(.999) = 52
    // TM(52) = .999x	
    
    // need to handle 1.f which is not valid
    sdr = min(sdr, .999f);
    return pow(ITM_Reinhard_Fixed(sdr, sqrt(4.0 / 27.0)), 2.0 / 3.0);
}
